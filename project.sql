...
SELECT country, product, price, count(product) AS No_products , SUM(price) AS total_sales from sales
GROUP BY country, product
HAVING No_products >= 3
ORDER BY count(product) DESC, SUM(price) DESC;

SELECT country,count(country) AS Top_countries FROM (SELECT country, product, price, count(product) AS No_products , SUM(price) AS total_sales from sales
GROUP BY country, product
HAVING No_products >= 3
...
SELECT price, payment_type,
     CASE
         ...
         WHEN payment_type = 'Visa' THEN 'Discount of 150'
         ELSE 'Discount of 50'
     END AS "Discount",
	 CASE
        WHEN payment_type = 'Mastercard' THEN price - 250
        WHEN payment_type = 'Visa' THEN price - 150
        ELSE price - 50
    END AS "Final_Price"
FROM sales
GROUP BY price, payment_type
ORDER BY price DESC;
